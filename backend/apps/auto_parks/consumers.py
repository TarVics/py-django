...
class AutoParkConsumer(GenericAsyncAPIConsumer):
    ...
    async def sender(self, event):
        # channel_layer.group_send(self.room_name, EVENT) -> sender.event
        await self.send_json(event)

    # @action() - user execute procedure from front-end
    # look in frontend\src\components\Chat\Chat.js
    @action()
